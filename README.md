...
Hyper Get’s desktop dashboard now ends with two backend-driven layers: a byte-weighted global progress bar and a compact status bar. URL entry remains in Add URL, Ctrl+V, browser integration, and the wildcard batch dialog; the former bottom Quick Download input was removed.

Pause All persists a disabled queue-execution state before pausing active work, so queued downloads cannot be started by completion callbacks. Start All re-enables scheduling. Settings use the operating system Downloads directory resolved by Tauri and validate custom folders before saving.
